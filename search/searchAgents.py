...
class DeceptiveSearchAgentpi2(SearchAgent):
    ...
    def registerInitialState(self, state):
        ...
        self.actions = []
        false_goals = state.getCapsules()
        true_goal = state.getFood().asList()[0]
        problem1 = PositionSearchProblem(state, start=state.getPacmanPosition(), goal=true_goal, warn=False, visualize=False)
        a_action = search.astar(problem1)
        a = len(a_action)
        false_action = []
        beta = float("inf")
        min_state = ()
        ...
        for false_goal in false_goals:
            problem2 = PositionSearchProblem(state, start=false_goal, goal=true_goal, warn=False, visualize=False)
            problem3 = PositionSearchProblem(state, start=state.getPacmanPosition(), goal=false_goal, warn=False, visualize=False)
            b_actions = search.astar(problem3)
            c_actions = search.astar(problem2)
            c = len(c_actions)
            b = len(b_actions)
            if (a + c - b) / 2 < beta:
                beta = (a + c - b) / 2
                false_action = c_actions
                min_state = false_goal    
        for i in range(0, len(false_action) - int(beta)):
            min_state = self.getNextState(min_state, false_action[i])
        problem_min = PositionSearchProblem(state, start=state.getPacmanPosition(), goal=min_state, warn=False, visualize=False)
        result = search.astar(problem_min)
        problem_min2 = PositionSearchProblem(state, start=min_state, goal=true_goal, warn=False, visualize=False)
        result += search.astar(problem_min2)
        self.actions = result
        return result
    ...
    def registerInitialState(self, state):
        ...
        for false_goal in false_goals:
            ...
            if (a + c - b) / 2 < beta:
                ...
                false_action = c_actions
                min_state = false_goal    
        false_goal = min_state
        for i in range(0, len(false_action) - int(beta)):
            min_state = self.getNextState(min_state, false_action[i])
        problem_min = PositionSearchProblem(state, start=state.getPacmanPosition(), goal=min_state, warn=False, visualize=False)
        problem_min.heuristicInfo = (true_goal, false_goal)
        result = search.astar(problem_min, pi3heuristic)
        problem_min2 = PositionSearchProblem(state, start=min_state, goal=true_goal, warn=False, visualize=False)
        result += search.astar(problem_min2)
        self.actions = result
        return result
...
def pi3heuristic(state, problem):
    position = state
    h = util.manhattanDistance(position, problem.goal)
    food, capsule = problem.heuristicInfo
    if util.manhattanDistance(position, food) < util.manhattanDistance(position, capsule):
        h = 2 * h
    return h
...
def foodHeuristic(state, problem):
    ...
    for food in food_coordinates:
        pac_food_distances.append((food, mazeDistance_with_capsule(position, food, capsules, problem)))       
        
    min_pos, min_distance = pac_food_distances[0]
    for (pos,distance) in pac_food_distances:
    ...
    h += min_distance
    position = min_pos
    food_coordinates.remove(min_pos)
    pac_food_distances = []
    ...
    for food in food_coordinates:
        pac_food_distances.append((mazeDistance_with_capsule(position, food, capsules, problem)))       
    
    max_distance = pac_food_distances[0]
    for distance in pac_food_distances:
...
def mazeDistance_with_capsule(point1, point2, capsules, problem):
    
    x1, y1 = point1
    prob = PositionSearchProblem(problem.startingGameState, start=point1, goal=point2, costFn= lambda x : 0 if x in capsules else 1, warn=False, visualize=False)
    result = search.astar(prob)
    distance = len(result)
